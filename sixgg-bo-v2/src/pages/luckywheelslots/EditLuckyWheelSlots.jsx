...
import { Button, Form, Input, Modal, Select, Switch, Row, Col, Image } from 'antd';
...
import { useGetLuckyWheelSlotsIDQuery, useUpdateLuckyWheelSlotsMutation } from '../../features/luckywheelslots/luckyWheelSlotsApiSlices';

const LuckyWheelSlotsForm = ({ onFormInstanceReady, id, apiErrors, t }) => {
    ...
    const { data: record, isLoading: recordLoading,} = useGetLuckyWheelSlotsIDQuery({ id: id });
    if(recordLoading){
...
const LuckyWheelSlotsFormModal = ({ open, onUpdate, onCancel, apiErrors, id, t }) => {
    const [formInstance, setFormInstance] = useState();

return (
    <Modal
        open={open}
        title={t("luckywheelslot.Update Lucky Wheel Slots")}
...
const EditLuckyWheelSlots = ({ id, t }) => {
    const [open, setOpen] = useState(false);
    const [apiErrors, setApiErrors] = useState([])
    const [UpdateLuckyWheelSlots, { isLoading }] = useUpdateLuckyWheelSlotsMutation();
    ...
    try {
      values.id = id;
      const data = await UpdateLuckyWheelSlots(values).unwrap();
      dispatch(notification({notification_status: 'success', notification_message: `${t('notisuccess.Lucky Wheel Slot Updated Successfully')}`}));
      setOpen(false);
    } catch (error) {
...
export default EditLuckyWheelSlots;