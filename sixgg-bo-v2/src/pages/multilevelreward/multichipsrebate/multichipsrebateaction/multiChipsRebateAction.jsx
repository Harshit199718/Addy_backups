...
const MultiChipsRebateAction = ({ record, t }) => {
    const [MultiChipsRebateAction, { isLoading }] = useUpdateChipsRebateMutation();
    const [apiErrors, setApiErrors] = useState([])
    const dispatch = useDispatch();

    const onMultiChipsRebateAction = async (values, state) => {
        values = {...values, state: state, player_id: values.player, approved_at: getCurrentTime()  }
        setApiErrors([])
        try {
          const data = await MultiChipsRebateAction(values).unwrap();
...
export default MultiChipsRebateAction;