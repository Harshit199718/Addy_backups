...
const MultiBonusesAction = ({ record, t }) => {
    const [remark, setRemark] = useState('')
    const [MultiBonusesAction, { isLoading }] = useUpdateBonusesMutation();
    const [apiErrors, setApiErrors] = useState([])
    const dispatch = useDispatch();

    const onMultiBonusesAction = async (values, state) => {
        values = {...values, remark: remark, state: state, player_id: values.player}
        setApiErrors([])
        try {
          const data = await MultiBonusesAction(values).unwrap();
...
export default MultiBonusesAction;