...
import { useAddMissionsMutation } from '../../features/missions/missionsApiSlices';
...
const FormList = ({ onFormInstanceReady, apiErrors, t }) => {
  ...
  const initialValues = {
    ...
    max_coupon: 0,
    max_bonus_amount: 0,
    token_expired_days: 0,
    bonus_percent: 0,
    bonus_amount: 0,
    x_count_checkin: 0,
    x_count_deposit_checkin: 0,
    x_amount_deposit_checkin: 0,
    min_deposit: 0,
    min_withdrawal: 0, 
    min_bonus: 0,
    limit_per_user: 0,
    x_count_deposit: 0,
    x_count_withdrawal: 0,
    x_count_bonus: 0,
    min_turnover_amount: 0,
    x_turnover_amount: 0,
    x_amount_deposit: 0,
    x_amount_withdrawal: 0,
    x_amount_bonus: 0,
    category: 'deposit',
    type_offer: 'token'
  }
...
const CreateMissions = ({ t }) => {
  const [open, setOpen] = useState(false);
  const [apiErrors, setApiErrors] = useState([])
  const [Create, { isLoading }] = useAddMissionsMutation();
  ...
  const onCreate = async (values) => {
    ...
    try {
      ...
      dispatch(notification({notification_status: 'success', notification_message: `${t('notisuccess.Mission Created Successfully')}`}));
      setOpen(false);
    } catch (error) {
