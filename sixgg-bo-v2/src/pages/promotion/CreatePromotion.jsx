...
import { Button, Col, Divider, Form, Input, InputNumber, Modal, Row, Select, Switch, Tag } from 'antd';
...
import { useAddPromotionMutation } from '../../features/promotion/promotionApiSlices';
import { formattedDate, formattedTime } from '../../components/convertDate';
...
const FormList = ({ onFormInstanceReady, apiErrors, t }) => {
  ...
  const [sites, promo_type, selected_product, promo_template, description, header_description, reward_type, rebate_product] = [
    'sites', 
    'promo_type', 
    'selected_product',
    'promo_template',
        ...
        <Col xs={24} sm={24} md={promo_type === 'DF' ? 12 : 8}>
          ...
          <Row gutter={[16, 16]}>
            ...
            <Col xs={24} sm={24} md={promo_type === 'DF' ? 0 : 12} style={{...CTCConditionChecking(promo_template, 'group')}}>
              <ReferencePromotionGroupField name="group" label={t("common.Promotion Group")} apiErrors={apiErrors && apiErrors.group} filterProp={{ sites: sites }}/>
            </Col>
            }
          ...
          <Row gutter={[16, 16]}>
            ...
            <Col xs={24} sm={24} md={12} style={{...CTCConditionChecking(promo_template, 'limit_per_user')}}>
              ...
              <Form.Item
                ...
                hasFeedback
              >
                <InputNumber placeholder={t("common.Limit per player can redeem")} style={{ width: '100%' }} min={0}/>
              </Form.Item>
            </Col>
          </Row>
        ...
        <Col xs={24} sm={24} md={promo_type === 'DF' ? 0 : 8}>
          ...
          <Row gutter={[16, 16]} style={{...CTCConditionChecking(promo_template, 'is_reward')}}>
              ...
              <Col span={24}>
                ...
                <ReferenceProductTransferField name="rebate_product" label={null} apiErrors={apiErrors && apiErrors.rebate_product} 
...
const FormModal = ({ open, onCreate, onCancel, apiErrors, t }) => {
  ...
  return (
    <Modal
      open={open}
      title={t("common.Create Promotion")}
...
const CreatePromotion = ({ t }) => {
  const [open, setOpen] = useState(false);
  const [apiErrors, setApiErrors] = useState([])
  const [Create, { isLoading }] = useAddPromotionMutation();
  ...
  const onCreate = async (values) => {
    ...
    if(values.image){
      ...
      dispatch(notification({notification_status: 'error', notification_message: `${t('notierror.Please Upload Images first')}`}));
    }
    ...
    try {
      ...
      dispatch(notification({notification_status: 'success', notification_message: `${t('notisuccess.Promotion Created Successfully')}`}));
      setOpen(false);
    } catch (error) {
