...
import { Button, Col, Divider, Form, Input, InputNumber, Modal, Switch, Tag } from 'antd';
...
import { useGetRankRatesIDQuery, useUpdateRankRatesMutation } from '../../features/rank/rankRatesApiSlices';
...
const FormList = ({ onFormInstanceReady, id, apiErrors, site, t }) => {
    ...
    <Form layout="vertical" form={form} name="form_in_modal" initialValues={initialValues}>
      <Col span={24}>
        ...
        <ReferenceProductTransferField 
          name="selected_product" label={null} 
          apiErrors={apiErrors && apiErrors.product} 
          filterProp={{ sites: site, category: [product_category] }} 
          targetKeys={selected_product && selected_product}/>
        </>
        )}
        <Form.Item
          name="rate"
...
const FormModal = ({ open, onUpdate, onCancel, apiErrors, id, site, t }) => {
  const [formInstance, setFormInstance] = useState();
  
  return (
    <Modal
      open={open}
      title={t("common.Update Rank Rate")}
...
const EditRankRate = ({ id, site, t }) => {
  const [open, setOpen] = useState(false);
  const [apiErrors, setApiErrors] = useState([])
  const [Update, { isLoading }] = useUpdateRankRatesMutation();
  ...
  const onUpdate = async (values) => {
    ...
    try {
      ...
      dispatch(notification({notification_status: 'success', notification_message: `${t('notisuccess.Rank Rate Updated Successfully')}`}));
