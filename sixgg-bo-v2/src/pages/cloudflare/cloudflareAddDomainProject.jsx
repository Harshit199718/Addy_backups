...
import { useCloudflareAddDomainProjectMutation } from '../../features/cloudflareconfig/cloudflareConfigApiSlices';
            ...
            <Col span={24}>
                <Form.Item
                    ...
                    rules={[
                        {
                            required: true,
                            message: `${t('requiredmessage.Please input the domain!')}`,
                        },
                    ]}
                    hasFeedback
                    >
...
const AddDomainFormModal = ({ open, onCreate, onCancel, apiErrors, record, successMessage, t }) => {
  ...
  return (
    <Modal
      open={open}
      title={record && <span>${t("cloudflare.Add Domain to Project")} </span>}
      okButtonProps={{
        autoFocus: true,
      }}
      footer={
...
const CloudflareAddDomainProject = ({ record, t }) => {
    const [open, setOpen] = useState(false);
    const [apiErrors, setApiErrors] = useState([])
    const [successMessage, setSuccessMessage] = useState(null)
    const [AddDomain, { isLoading }] = useCloudflareAddDomainProjectMutation();
    ...
    return (
        ...
        <Button type="primary" onClick={() => setOpen(true)}  style={{ width: '100%', marginTop: 10 }}>
          {t("cloudflare.Add Domain to Project")}
        </Button>
        <AddDomainFormModal
          open={open}
...
export default CloudflareAddDomainProject;