...
import BonusesToolBar from './bonusesToolBar';
...
import { filterBonusesType } from '../../../customField/customOption';
import { convertTtypeOption } from '../../../components/generalConversion';
import BonusesAction from './bonusesaction/bonusesAction';
import PermissionsAuth from '../../../components/permissionAuth';
import { useTranslation } from 'react-i18next';

const BonusesList = ( id ) => {
  ...
  const defaultTtype = convertTtypeOption([filterBonusesType(t)[0].value, filterBonusesType(t)[1].value]);
  ...
  return (
    PermissionsAuth.checkPermissions('list', 'view_bonus',
    <div>
      <BonusesToolBar isLoading={isFetching} t={t} />
        <Table
          rowKey="id"
          columns={[
            {
            ...
            },
            {
              ...
              key: 'player',
              render: (player) => (
                <ReferencePlayerListingField 
                filterProp={{id: [player]}} 
                showCustomerName={PermissionsAuth.checkPermissions('others', 'view_player_full_name_transaction', true)}
                />
              ),
            },
            {
            ...
            ...(shouldShowActionColumn ? [
              {
                ...
                align: 'center',
                width: 600,
                hidden: PermissionsAuth.checkPermissions('button', 'change_bonus', false),
                render: (record) => {
                  const { state } = record;
                  if (state) {
                    return (
