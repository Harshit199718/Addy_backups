...
import { useDropTransactionDepositActionMutation } from '../../../features/transaction/dropTransactionDeposit/dropTransactionDepositApiSlices';

const DropTransactionDepositAction = ({ record, t }) => {
    const [form] = Form.useForm();
    const [remark, setRemark] = useState('')
    const [DropTransactionDepositAction, { isLoading }] = useDropTransactionDepositActionMutation();
    const [apiErrors, setApiErrors] = useState([])
    const dispatch = useDispatch();

    const onDropTransactionDepositAction = async (values, state) => {
        values = {...values, remark: remark, state: state}
        setApiErrors([])
        try {
          const data = await DropTransactionDepositAction(values).unwrap();
          dispatch(notification({notification_status: 'success', notification_message: `${t("common.Drop Transaction Deposit")} ${state} ${t("common.Successfully")}`}));
        } catch (error) {
    ...
    return(
        <Form form={form} name={`${record?.txid}_dtd_form`}>
        <Col>
            ...
            {record.state === 'pending' && 
            PermissionsAuth.checkPermissions('others', 'change_droptransactiondeposit', 
            <Row>
                <Popconfirm
                    ...
                    onConfirm={() => onDropTransactionDepositAction(record, 'approved')}
                >
                    <Button 
                ...
                <Popconfirm
                    ...
                    okText={t("common.Yes")}
                    cancelText={t("common.No")}
                    onConfirm={() => onDropTransactionDepositAction(record, 'rejected')}
                >
                    <Button 
                        icon={<CloseOutlined />}
                        type="primary" 
...
export default DropTransactionDepositAction;